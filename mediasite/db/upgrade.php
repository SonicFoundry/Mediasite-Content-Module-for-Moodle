...
function xmldb_mediasite_upgrade($oldversion=0) {
    ...
    if ($result && $oldversion <= 2017020100) {
        $result = mediasite_upgrade_from_2016041803($oldversion, $dbman, $plugin);
    }
...
function mediasite_upgrade_from_2012032900($oldversion, $dbman, $plugin) {
    ...
    foreach($configrecords as $configrecord) {
            if($configrecord->name == 'mediasite_username') {
                    ...
                    //$siterecord['endpoint'] = preg_replace('/6_1_7\/?$/', 'main', $configrecord->value);
    ...
    if(array_key_exists("endpoint", $siterecord) && array_key_exists("username", $siterecord) && array_key_exists("password", $siterecord)) {
            // TODO: get version of Mediasite (6 or 7)
            $soapclient = Sonicfoundry\MediasiteClientFactory::MediasiteClient('soap',$siterecord['endpoint'], $siterecord['username'], $siterecord['password'], null);
            ...
            if(preg_match('/(6|7)\.(\d+)\.(\d+)/i', $version, $matches)) {
                    if($matches[1] == 6) {
                            $client = Sonicfoundry\MediasiteClientFactory::MediasiteClient('soap',$siterecord['endpoint'], $siterecord['username'], $siterecord['password'], null);
                            $apikeyneeded = false;
                            $siterecord['siteclient'] = 'soap';
                    }
                    else if($matches[1] == 7) {
                            $client = Sonicfoundry\MediasiteClientFactory::MediasiteClient('odata',$siterecord['endpoint'], $siterecord['username'], $siterecord['password'], null);
                            $apikeyneeded = true;
    ...
    try{
            ...
            if($mediasite_rs->valid()){
                    foreach ($mediasite_rs as $mediasiterecord) {
                            $record = new stdClass();
...
function mediasite_upgrade_from_2016041803($oldversion, $dbman, $plugin) {
    ...
    // 1. Add new column (sofotags)
    // 2. Copy data (tags --> sofotags)
    // 3. Remove old column (tags)
    $contenttable = new xmldb_table('mediasite');
