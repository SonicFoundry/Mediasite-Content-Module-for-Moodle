...
class mod_mediasite_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        if (isset($cm->id)) {
            ...
            $mform->addElement('select', 'displaymode', get_string('mode', 'mediasite'), $formoptions, array('onchange' => 'javascript:toggleEmbedModeChange(this.value);'));
            ...
            $mform->addElement('html', '<script type="text/javascript">setTimeout(function () { toggleEmbedModeChange("'.$mediasite->displaymode.'") }, 50);</script>');
        ...
        $mform->addElement('textarea', 'sofotags_display', get_string('sofotags', 'mediasite'), array('class' => 'sofo-embed mediasite-readonly sofo-embed-type-MetadataOnly sofo-embed-type-MetadataPlusPlayer',
                                                                                  'wrap' => "virtual",
                                                                                  'rows' => 3,
                                                                                  'cols' => 100,
                                                                                  'onfocus' => 'this.blur();'));
    ...
    function validation($data, $files)
    {
        ...
        if (mediasite_has_value($data['resourceid']) && mediasite_has_value($data['siteid']) && mediasite_has_value($data['resourcetype']) && mediasite_has_value($data['displaymode'])) {
            // good, proceed
        } else {
            // bad, blow up
